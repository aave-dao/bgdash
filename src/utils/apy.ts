...
import { toWad, wadMul } from './math/ray-math';
...
export const calculateRewardApy = (rewardData: RewardDataToCalculateApy) => {
  ...
  const rewardPriceWad = toWad(
    rewardData.token.price,
    rewardData.token.priceFeedDecimals,
  );
  const usdPerSecond =
    (rewardPriceWad * emissionPerSecond) /
    10n ** BigInt(rewardData.token.decimals);
...
export const getCurrentEmission = (data: RewardDataToCalculateApy): bigint => {
  ...
  const emissionScaled = getEmissionPerSecondScaled(
    maxEmissionScaled,
...
const getEmissionPerSecondScaled = (
  ...
  const params = calculateEmissionParams(
...
function slopeCurve(
  ...
  const emissionDecrease =
    (maxEmission * totalAssets * ONE_E18) / targetLiquidity;
  return (
    ((2n * maxEmission * ONE_E18 - emissionDecrease) * totalAssets) /
    targetLiquidity
  );
}
...
function linearDecrease(p: EmissionParams, totalAssets: bigint): bigint {
  const num =
    (p.maxEmission - p.flatEmission) * (totalAssets - p.targetLiquidity);
