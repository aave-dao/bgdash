...
export const toRay = (a: bigint, decimals: number) => {
  ...
  throw new Error(
    `Passed decimals is greater than ${RAY_PRECISION}, it is not possible to convert to Ray yet`,
  );
};
...
export const toWad = (a: bigint, decimals: number) => {
  ...
  throw new Error(
    `Passed decimals is greater than ${WAD_PRECISION}, use toRay instead`,
  );
};
...
export const fromWad = (amountWAD: bigint, targetDecimals: number) => {
  ...
  throw new Error(
    `Passed decimals is greater than ${WAD_PRECISION}, use fromRay instead`,
  );
};
